...
`packaging/windows` artik ayni `restotm-windows-host-v1` snake_case semasini uretir: exact top-level alanlar, alti child'in sirali dependency grafigi, `environment/file_environment/secret_environment`, sabit portlar, `values` map'li DPAPI store ve config/secret SHA-256 + ACL policy bagli `bootstrap-receipt.json` birebir eslestirildi. Node statik contract testi iki ornek topology arasindaki drift'i yakalar.
...
## Bootstrap helper durumu: bilerek fail-closed
...
Bilinmeyen/duplicate flag, traversal, credential/query iceren URL, duplicate/privileged port veya sabit PostgreSQL/gateway kontrati ihlali reddedilir.

Bu foundation'da native ACL olusturma + DPAPI `CryptProtectData` + atomic config/receipt yazma backend'i tamamlanmamistir. Bu nedenle `restotm-installer-bootstrap` dogru argumanlarla bile **exit code 78** doner; provisioning basariliymis gibi davranmaz. Runtime tarafinda DPAPI `CryptUnprotectData` implementasyonu vardir, fakat provisioning backend'i ve ACL dogrulamasi kapanmadan bu binary musteri artifact manifestine alinmamalidir.
...
- unavailable bootstrap backend'in false success vermemesi;
...
- Native provisioning backend ve DACL verification tamamlanmadi; helper exit 78 donuyor.
...
- PostgreSQL/API/admin/waiter/print/gateway gercek Windows artifact seti yok; LAN gateway executable'i henuz uretilmedi.
