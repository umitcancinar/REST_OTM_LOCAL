...
Teslim klasorunde setup ile ayni sertifikayla Authenticode imzalanmis destek araclari bulunur. `Repair-RestOtmHost.ps1` veri/secret silmeden receipt, native bootstrap, ACL, servis recovery, dar firewall ve canli child sagligini dogrular/onarir. `Get-RestOtmDiagnosticBundle.ps1` yalniz sinirli servis/port/firewall/hash/log bilgisini toplar; lisans, e-posta, bearer, parola, token ve DB URL degerlerini ZIP olusmadan once redakte eder. `Get-RestOtmAccessAddresses.ps1` yalniz calistirildigi kurulu musteri bilgisayarinin config hostname'ini ve private IPv4 adreslerini masaustune yazar. `Find-RestOtmNetworkDevices.ps1` yalniz Private profildeki dogrudan bagli RFC1918 /24 diliminde RAW 9100 portunu sinirli paralellikle tarar; public hedef, serbest port, MAC veya secret kabul/rapor etmez.
