...
Script kaynakları derler, API'nin tam npm/Prisma çalışma closure'ını çıkarır, Rust servis/bootstrap/launcher PE dosyalarını üretir, kendi PE dosyalarını imzalar, üçüncü taraf PE imzalarını denetler, payload'ı denetler ve MSI + imzalı Burn setup üretir. Bir adım başarısızsa müşteri dosyası üretmez.
...
Bu test servis, delayed auto-start, restart politikası, DPAPI secret envelope, hash bağlı bootstrap receipt, dar firewall, yalnız-loopback iç servisler, gateway ve web endpointlerini kontrol eder. Başarılı olursa aynı imzalı setup hash değiştirmeden `01_MUSTERIYE_VERILECEK` klasörüne kopyalanır.

## 4. Müşteriye verilecekler

Yalnız `WINDOWS_KURULUM\01_MUSTERIYE_VERILECEK` klasörünü verin. Kaynak kod, `build`, sertifika, PFX, private key, `.env`, veritabanı veya log klasörünü vermeyin.
