...
export function twoColumnLine(left: unknown, right: unknown, columns: number): string {
 ...
 */
export function hardWrap(value: unknown, width: number): string[] {
...
export function scaledColumns(columns: number, scale: Scale): number {
  return Math.max(1, Math.floor(columns / scale));
}

/** Onizlemede satiri kagit genisligine hizalar (ESC a ile ayni sonuc). */
export function padForAlign(text: string, columns: number, align: Align): string {
  if (text.length >= columns) return text;
  const space = columns - text.length;
  if (align === 'right') return ' '.repeat(space) + text;
  if (align === 'center') return ' '.repeat(Math.floor(space / 2)) + text;
  return text;
}

const CP857_MAP: Record<string, number> = {
 ...
 */
export function toCP857(text: string): Uint8Array {
  const bytes: number[] = [];
