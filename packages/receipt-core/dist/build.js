...
exports.cutClearanceMm = cutClearanceMm;
exports.buildReceiptDoc = buildReceiptDoc;
 ...
 * davranirsa yaniltir.
 */
function cutClearanceMm(kind, bottomMarginMm) {
...
class DocWriter {
    ...
    width(key) {
        return (0, text_1.scaledColumns)(this.columns, this.styleOf(key).scale);
    }
    ...
    write(key, value) {
        ...
        if (!style.visible)
            return;
        for (const line of (0, text_1.wrapText)(value, this.width(key))) {
    ...
    writeLine(key, value) {
        ...
        if (!style.visible)
            return;
        for (const line of (0, text_1.hardWrap)(value, this.width(key))) {
            this.lines.push({ text: line, align: style.align, bold: style.bold, scale: style.scale, source: key });
        }
    }
...
function itemDisplayName(item, treatTag) {
    let name = String(item.name || '').toUpperCase();
    if (item.isTreat || (item.notes && item.notes.includes('[İKRAM]')))
        name += ` ${treatTag}`;
    if (item.portionOption && item.portionOption !== 'Normal')
        name += ` (${item.portionOption})`;
    return name.trim();
}
function writeItems(writer, items, layout, columns, showPrices) {
    const itemStyle = layout.elements.item;
    const width = (0, text_1.scaledColumns)(columns, itemStyle.scale);
    for (const item of items) {
        ...
        if (itemStyle.scale === 1) {
            writer.writeRaw('item', (0, text_1.itemColumns)(name, quantity, priceText, columns, layout.qtyWidth, showPrices ? layout.priceWidth : 0));
        }
        else {
            for (const line of (0, text_1.wrapText)(`${quantity}x ${name}`, width)) {
                writer.writeRaw('item', line);
            }
            if (priceText) {
                writer.lines.push({
                    text: (0, text_1.fitText)(priceText, width),
                    align: 'right',
                    bold: itemStyle.bold,
 ...
 */
function buildReceiptDoc(input) {
    const layout = input.layout;
    const totalColumns = types_1.PAPER_COLUMNS[layout.paperWidth];
    const mmPerChar = types_1.PRINTABLE_MM[layout.paperWidth] / totalColumns;
    ...
    if (layout.headerText)
        writer.write('header', layout.headerText);
    if (layout.subHeaderText)
        writer.write('subHeader', layout.subHeaderText);
    writer.separator();
    if (input.isCancel) {
    ...
    if (input.customer && writer.visible('customer')) {
        ...
        if (input.customer.phone)
            writer.write('customer', `${layout.labels.phone} ${input.customer.phone}`);
        if (input.customer.address)
            writer.write('customer', `${layout.labels.address} ${input.customer.address}`);
        writer.separator(layout.itemSeparatorChar);
    }
    // ---- Siparis notu -----------------------------------------------------
    if (input.notes) {
    ...
    if (layout.inlineDateMasa && tableStyle.scale === 1 && writer.visible('dateTime') && tableStyle.visible) {
        ...
        writer.writeRaw('dateTime', (0, text_1.twoColumnLine)(`${date} ${time}`, tableText, columns));
    }
    else {
    ...
    if (input.waiterName)
        writer.writeLine('waiter', `${layout.labels.waiter} ${input.waiterName}`);
    writer.separator();
    // ---- Sutun basliklari -------------------------------------------------
    if (layout.elements.item.scale === 1 && writer.visible('columnsHeader')) {
        writer.writeRaw('columnsHeader', (0, text_1.itemColumns)(layout.labels.colProduct, layout.labels.colQty, layout.labels.colAmount, columns, layout.qtyWidth, showPrices ? layout.priceWidth : 0));
        writer.separator();
    }
    // ---- Urunler ----------------------------------------------------------
    ...
    if (showPrices) {
        ...
        if (writer.visible('total')) {
            writer.writeRaw('total', (0, text_1.twoColumnLine)(writer.labelFor('total', layout.labels.total), money(totalAmount, layout.labels.currency), writer.width('total')));
        }
        ...
        if (input.payments && input.payments.length > 0 && writer.visible('payments')) {
            ...
            for (const payment of input.payments) {
                ...
                writer.writeRaw('payments', (0, text_1.twoColumnLine)(label, money(Number(payment.amount), layout.labels.currency), writer.width('payments')));
                paid += Number(payment.amount) || 0;
            }
            if (writer.visible('remaining')) {
                writer.separator();
                writer.writeRaw('remaining', (0, text_1.twoColumnLine)(writer.labelFor('remaining', layout.labels.remaining), money(totalAmount - paid, layout.labels.currency), writer.width('remaining')));
            }
        }
    }
    ...
    if (layout.showPaidItems && paidItems.length > 0 && writer.visible('paidItems')) {
        ...
        for (const item of paidItems) {
            writer.writeRaw('paidItems', (0, text_1.itemColumns)(itemDisplayName(item, layout.labels.treatTag), String(item.quantity ?? 0), showPrices ? money(Number(item.price ?? 0) * Number(item.quantity ?? 0), layout.labels.currency) : '', writer.width('paidItems'), layout.qtyWidth, showPrices ? layout.priceWidth : 0));
        }
    }
    writer.separator();
    if (layout.footerText)
        writer.write('footer', layout.footerText);
    // ---- Yan bosluk -------------------------------------------------------
    ...
    const lines = sideMarginChars > 0
        ? writer.lines.map((line) => {
            const scaledCols = (0, text_1.scaledColumns)(columns, line.scale);
            const prefixChars = Math.max(0, Math.round(sideMarginChars / line.scale));
            const justified = (0, text_1.padForAlign)(line.text, scaledCols, line.align);
            return { ...line, text: ' '.repeat(prefixChars) + justified, align: 'left' };
        })
        : writer.lines;
    return {
