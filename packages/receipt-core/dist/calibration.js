...
exports.CALIBRATION_INFO = void 0;
exports.buildCalibrationDoc = buildCalibrationDoc;
...
const MM_PER_LINE = types_1.LINE_HEIGHT_DOTS / types_1.DOTS_PER_MM; // 24 nokta = 3 mm
 ...
 */
function buildCalibrationDoc(layout, kind = 'BILL') {
    ...
    const bottomMarginMm = (0, build_1.cutClearanceMm)(kind, layout.bottomMarginMm);
    const lines = [];
    lines.push(line('KALIBRASYON FISI', { bold: true, align: 'center' }));
    lines.push(line('-'.repeat(columns)));
    lines.push(line(`Motor surumu : v${types_1.RENDER_ENGINE_VERSION}`));
    lines.push(line(`Kagit        : ${layout.paperWidth} mm / ${columns} sutun`));
    lines.push(line(`Satir yuks.  : ${MM_PER_LINE.toFixed(2)} mm (${types_1.LINE_HEIGHT_DOTS} nokta)`));
    lines.push(line('-'.repeat(columns)));
