  ...
  const handleCompletePayment = async (method: PaymentMethod) => {
    ...
    try {
      ...
      if (isFullyPaid) {
        ...
        // DIKKAT: bu panelde '/' GIRIS ekranidir (src/app/page.tsx).
        // Buraya yonlendirmek garsonu odeme sonrasi sistemden atiyordu.
        router.push('/tables');
      } else {
    ...
    <div className={styles.container} ref={containerRef}>
      ...
      >
        <div className={styles.leftHeader}>
          ...
          <div className={styles.headerActions}>
            {/* Print button — mutfak/adisyon fisi secenekleri */}
            {activeOrder && (
              <button
            ...
            {/* Payment button — yazdirmadan bagimsiz, odeme al/kapat */}
            {activeOrder && (
              <button
                onClick={() => {
                ...
                <div className={styles.actionsDropdown}>
                  ...
                  {activeOrder && (
                    <button
                      ...
                      style={{ opacity: isCancellingOrder ? 0.5 : 1 }}
                      onClick={() => {
                        setIsActionsMenuOpen(false);
      ...
      {showPaymentModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 100000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(8px)' }}>
          <div style={{ width: '100%', maxWidth: 440, maxHeight: 'calc(100dvh - 40px)', overflowY: 'auto', background: 'var(--bg-surface)', borderRadius: 'var(--radius-2xl)', padding: 32 }}>
                  ...
                  {PAYMENT_METHOD_OPTIONS.map(method => {
                    ...
                    return (
                      <button
                        ...
                        style={{ minHeight: 104, padding: '12px 8px', borderRadius: 'var(--radius-xl)', border: '2px solid var(--border)', background: 'var(--bg-elevated)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 9, fontWeight: 800, fontSize: '0.78rem', cursor: 'pointer', color: 'var(--text-primary)', textAlign: 'center' }}
                      >
