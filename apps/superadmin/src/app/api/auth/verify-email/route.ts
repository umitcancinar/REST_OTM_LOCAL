...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(controlApiUrl("auth/superadmin/mfa/verify"), { method: "POST", headers: { "Content-Type": "application/json", "User-Agent": req.headers.get("user-agent") || "REST_OTM Superadmin", "x-rest-otm-service-secret": process.env.SUPERADMIN_BFF_SERVICE_SECRET }, body: JSON.stringify({ challengeId: pending.id, code: normalizedCode }), cache: "no-store", redirect: "error" });
    const payload = await readControlApiJson<{ message?: string; data?: { tokens?: { accessToken?: string; refreshToken?: string }; user?: { role?: string } } }>(upstream, "superadmin-mfa-verify");
    if (!upstream.ok) {
    ...
    if (!accessToken || !refreshToken || data?.user?.role !== "SUPER_ADMIN") {
