...
export function turnstileAction() {
  return ACTION;
}
...
export async function verifyTurnstile(
  ...
  // Production must never silently fall back to Cloudflare test credentials or
  // accept a response from an unlisted hostname.
  if (!secret || (production && allowedHostnames.size === 0)) {
    return { ok: false, reason: "service-unconfigured" };
  }
  ...
  try {
    ...
    if (!result.success) {
      return {
        ok: false,
        reason: result["error-codes"]?.join(",") || "challenge-failed",
      };
    }
    if (production && result.action !== ACTION) {
