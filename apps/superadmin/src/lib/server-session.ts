...
type PendingMfa = { id: string; expiresAt: number };
...
export function createPendingMfa(input: { id: string; expiresAt: string }): PendingMfa { const parsed = Date.parse(input.expiresAt); if (!/^[A-Za-z0-9_-]{32}$/.test(input.id) || !Number.isFinite(parsed)) throw new Error("Invalid MFA challenge"); return { id: input.id, expiresAt: Math.min(parsed, Date.now() + MFA_TTL_SECONDS * 1000) }; }
...
export function pendingMfa(req: NextRequest) { const pending = open<PendingMfa>(req.cookies.get(PENDING_COOKIE)?.value); if (!pending || Date.now() > pending.expiresAt) return null; return pending; }
