...
function requireValue(name: string, devFallback: string): string {
  const value = process.env[name];
  if (!IS_PROD) return value || devFallback;
  if (!value) {
    startupErrors.push(`${name} tanimli degil.`);
...
const CORS_ORIGIN: string[] = (() => {
  ...
  if (!IS_PROD) return parsed.length > 0 ? parsed : DEV_ORIGINS;

  if (parsed.length === 0) {
    startupErrors.push(
      'CORS_ORIGIN tanimli degil — panel ve garson paneli adresleri virgulle ayrilmis olmali.',
    );
  }
  return parsed;
})();
