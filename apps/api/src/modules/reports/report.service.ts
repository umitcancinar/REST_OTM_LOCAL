...
export const reportService = {
  ...
  async getSummaryInRange(tenantId: string, start: string, end: string) {
    ...
    const paymentBreakdown = orders.reduce((acc, o) => {
      const trackedPayments = Array.isArray(o.payments)
        ? (o.payments as Array<{ method?: string; amount?: number }>)
        : [];
      let trackedSum = 0;
    ...
    // Waiter performance
    const waiterPerformance: Record<string, { name: string; orders: number; revenue: number }> = {};
    orders.forEach((o) => {
  ...
  async getRevenueByRange(tenantId: string, startDate: string, endDate: string) {
    const orders = await prisma.order.findMany({
      where: {
        tenantId,
  ...
  async getDepartmentStats(tenantId: string, date: string) {
